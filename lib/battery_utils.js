...
async function createBlepayloadArr(adapter, devicetype, device, channel, item, value, cmdObj) {
	let ctrl = '';
	let payloadArr = [];
	...
	switch (item) {
		case 'cd1':
			...
			payloadArr.push(makeCdCmd(value, cd2val));
			break;
		...
		case 'cd2':
			...
			ctrl = cmdObj['ble'][devicetype][channel]['cd']['ctrl'];
			payloadArr.push(makeCdCmd(cd1val, value));
			break;

		case 'lv':
			ctrl = cmdObj['ble'][devicetype][channel]['lv']['ctrl'];
			payloadArr.push(value.toString(16));
			break;

		case 'tun':
			ctrl = cmdObj['ble'][devicetype][channel]['tun']['ctrl'];
			payloadArr.push(makeTunCmd(value));
			break;
...
async function storepayloadArr(adapter, stateDictObj, stateObj, topic, payloadArr) {
	if (payloadArr) {
		try {
			...
			adapter.log.debug('store payloadArr ' + error);
		}
	} else {
...
exports.storepayloadArr = storepayloadArr;
exports.createBlepayloadArr = createBlepayloadArr;
