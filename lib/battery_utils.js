...
async function createBlePayload(adapter, devicetype, device, channel, item, value, cmdObj) {
	...
	switch (item) {
		case 'cd1':
			...
			ctrl = cmdObj['ble'][devicetype][channel][item]['cd'];
			payload = makeCdCmd(value, cd2val);
			break;
		...
		case 'cd2':
			...
			ctrl = cmdObj['ble'][devicetype][channel][item]['cd'];
			payload = makeCdCmd(cd1val, value);
			break;

		case 'lv':
			ctrl = cmdObj['ble'][devicetype][channel]['lv'];
			payload = value.toString(16);
			break;

		case 'tun':
			ctrl = cmdObj['ble'][devicetype][channel]['tun'];
			payload = makeTunCmd(value);
			break;

		case 'cs':
			ctrl = cmdObj['ble'][devicetype][channel]['lv'];
			payload = value === true ? '01' : '00';
			break;

		default:
			//update, deviceinfo, getssid
			ctrl = cmdObj['ble'][devicetype][channel][item];
			payload = '01';
			break;
	}
	const baseArr = [ '73', '06', '23', '08' ];
	const payHex = toHex(payload);
	const payloadArr = toArray(payHex);
